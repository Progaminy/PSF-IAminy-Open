...
Em 17 de julho de 2026, `pytest --collect-only -q` recolheu 1084 casos em 108
...
`testes/test_exercicio_real.py` sozinho contém 291 casos coletados (26,9% da
...
Passar 1084 casos também não prova:
...
`python3 ferramentas/auditar_testes.py` inspeciona a AST dos 108 ficheiros,
sem executar o código. Resultado de 17 de julho de 2026:

```text
763 funções test_*
1537 instruções assert
70 usos de pytest.raises
0 funções sem assert/pytest.raises explícito
0 grupos de corpos AST exatamente duplicados
0 ficheiros com erro de sintaxe
```

Os 763 corpos geram 1084 casos coletados porque parametrizações expandem uma
